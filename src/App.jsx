...
function fmtDate(iso) { if (!iso) return "—"; const [y,m,d] = iso.slice(0,10).split("-"); return `${d}/${m}/${String(y).slice(-2)}`; }
...
function buildAgendaCaso(caso, pasNombre) {
  const notas = [
    caso.nota            ? `📝 ${caso.nota}`                  : "",
    ...
    if (key === 'pas_casos') {
      ...
      Object.entries(val).forEach(([pas_id, casosList]) => {
        casosList.forEach(c => rows.push({
          ...
          carpeta_path: c.carpeta_path || null, compania: c.compania || null, fecha_siniestro: c.fecha_siniestro || null,
          presupuesto: c.presupuesto || null, primer_ofrecimiento: c.primer_ofrecimiento || null,
          segundo_ofrecimiento: c.segundo_ofrecimiento || null,
          fecha_carga: c.fecha_carga || null, fecha_reclamo: c.fecha_reclamo || null,
          fecha_ultimo_reclamo: c.fecha_ultimo_reclamo || null, fecha_ofrecimiento: c.fecha_ofrecimiento || null,
          fecha_reconsideracion: c.fecha_reconsideracion || null, fecha_aceptacion: c.fecha_aceptacion || null,
          fecha_firma: c.fecha_firma || null, fecha_pago: c.fecha_pago || null,
          fecha_cobro: c.fecha_cobro || null, fecha_mediacion: c.fecha_mediacion || null,
          fecha_inicio_juicio: c.fecha_inicio_juicio || null,
          monto_acordado: c.monto_acordado || null, plazo_pago: c.plazo_pago || null,
...
function CasoModal({ pasNombre, casoEdit, onClose, onSave, darkMode }) {
  const blank = { asegurado: "", estado: "iniciado", nota: "", compania: "", fecha_siniestro: "", fecha_derivacion: "", fecha_contacto_asegurado: "", fecha_inicio_reclamo: "", fecha_ultimo_movimiento: new Date().toISOString().slice(0, 10), monto_ofrecimiento: "", monto_cobro_asegurado: "", monto_cobro_yo: "", monto_comision_pas: "", recordatorio: "", notas_log: [] };
  ...
  const agregarNota = () => {
    ...
    const entry = { texto: nuevaNota.trim(), fecha: nuevaFecha, ts: Date.now() };
    setD(p => ({ ...p, notas_log: [...(p.notas_log || []), entry] }));
    setNuevaNota("");
    setNuevaFecha(new Date().toISOString().slice(0, 10));
  };
  const borrarNota = (ts) => setD(p => ({ ...p, notas_log: (p.notas_log || []).filter(n => n.ts !== ts) }));
  const iniciarEdicion = (n) => { setEditandoIdx(n.ts); setEditTexto(n.texto); setEditFecha(n.fecha); };
  const guardarEdicion = (ts) => {
    setD(p => ({ ...p, notas_log: (p.notas_log || []).map(n => n.ts === ts ? { ...n, texto: editTexto, fecha: editFecha } : n) }));
    setEditandoIdx(null);
  };
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.85)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 12, overflowY: "auto" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: darkMode ? "#0c1424" : "#fff", border: `1px solid ${darkMode ? "#1e3a5f" : "#e2e8f0"}`, borderRadius: 18, width: "100%", maxWidth: 560, padding: "24px 24px 28px", boxShadow: "0 32px 80px #000d", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <label>
            <div style={{ fontSize: 10, color: darkMode ? "#475569" : "#94a3b8", marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.8 }}>Compañía aseguradora</div>
            <input value={d.compania || ""} onChange={set("compania")} placeholder="Ej: Sancor Seguros" style={{ ...iStyle, fontSize: 13, padding: "7px 10px" }} />
          </label>
          <label>
            <div style={{ fontSize: 10, color: darkMode ? "#475569" : "#94a3b8", marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.8 }}>Fecha del siniestro</div>
            <input type="date" value={d.fecha_siniestro || ""} onChange={set("fecha_siniestro")} style={{ ...iStyle, fontSize: 13, padding: "7px 10px" }} />
          </label>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {(d.notas_log || []).length > 0 && (
            <div style={{ marginBottom: 12, maxHeight: 220, overflowY: "auto", paddingLeft: 4 }}>
              {[...(d.notas_log || [])].sort((a,b) => b.ts - a.ts).map((n) => (
                <div key={n.ts} style={{ display: "flex", gap: 10, marginBottom: 10, position: "relative" }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#6366f1", marginTop: 4, flexShrink: 0 }} />
                    <div style={{ width: 1, flex: 1, background: darkMode ? "#1e293b" : "#e2e8f0", marginTop: 3 }} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0, paddingBottom: 6 }}>
                    {editandoIdx === n.ts ? (
                      <div style={{ background: darkMode ? "#0f172a" : "#f8fafc", border: `1px solid #6366f188`, borderRadius: 8, padding: "8px 10px" }}>
                        <input type="date" value={editFecha} onChange={e => setEditFecha(e.target.value)} style={{ ...iStyle, fontSize: 11, padding: "4px 8px", marginBottom: 6 }} />
                        <textarea value={editTexto} onChange={e => setEditTexto(e.target.value)} rows={2} style={{ ...iStyle, resize: "vertical", marginBottom: 6 }} />
                        <div style={{ display: "flex", gap: 6 }}>
                          <button onClick={() => guardarEdicion(n.ts)} style={{ flex: 1, background: "#6366f1", border: "none", borderRadius: 6, color: "white", padding: "5px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Guardar</button>
                          <button onClick={() => setEditandoIdx(null)} style={{ flex: 1, background: darkMode ? "#1e293b" : "#e2e8f0", border: "none", borderRadius: 6, color: "#94a3b8", padding: "5px", cursor: "pointer", fontSize: 12 }}>Cancelar</button>
                        </div>
                      </div>
                    ) : (
                      <div style={{ background: darkMode ? "#0f172a" : "#f8fafc", border: `1px solid ${darkMode ? "#1e293b" : "#e2e8f0"}`, borderRadius: 8, padding: "8px 10px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                          <div style={{ fontSize: 11, color: "#64748b", fontWeight: 600 }}>{fmtDate(n.fecha)}</div>
                          <div style={{ display: "flex", gap: 4 }}>
                            <button onClick={() => iniciarEdicion(n)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, color: "#475569", padding: "1px 4px" }}>✏️</button>
                            <button onClick={() => borrarNota(n.ts)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, color: "#475569", padding: "1px 4px" }}>🗑</button>
                          </div>
                        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ display: "flex", gap: 8 }}>
              <input type="date" value={nuevaFecha} onChange={e => setNuevaFecha(e.target.value)} style={{ ...iStyle, width: "auto", flex: "0 0 140px", fontSize: 13 }} />
              <input value={nuevaNota} onChange={e => setNuevaNota(e.target.value)} onKeyDown={e => e.key === "Enter" && agregarNota()} placeholder="Describir acción..." style={{ ...iStyle, flex: 1 }} />
            </div>
            <button onClick={agregarNota} style={{ background: "#6366f1", border: "none", borderRadius: 8, color: "white", padding: "8px 14px", cursor: "pointer", fontWeight: 700, fontSize: 13 }}>+ Agregar acción</button>
          </div>
        </div>
...
function CasoCard({ caso, onEdit, onDelete, onDetalle, darkMode }) {
  ...
  const bg2   = darkMode ? "#222f42" : "#f1f5f9";
  const bor   = darkMode ? "#2d3f55" : "#e2e8f0";
    ...
    <div style={{ background: darkMode ? "#0f172a" : "#fff", border: `1px solid ${expanded ? "#22c55e77" : darkMode ? "#1e293b" : "#e2e8f0"}`, borderRadius: 13, marginBottom: 10, overflow: "hidden", transition: "border-color .2s" }}>
      ...
      {expanded && (
        <div style={{ borderTop: `1px solid ${darkMode ? "#1e293b" : "#e2e8f0"}`, padding: "14px 16px" }}>
          ...
          {casosFiltrados.map(c => <CasoCard key={c.id} caso={c} onEdit={onEditCaso} onDelete={onDeleteCaso} onDetalle={onDetalleCaso} darkMode={darkMode} />)}
...
function TabPortalUsuarios({ pas, derivadores, darkMode }) {
  ...
  const handleCrear = async () => {
    ...
    const { data: signData, error: signErr } = await supabase.auth.signUp({ email: email.trim(), password: pwd });
    if (signErr) { setError("Error al crear usuario: " + signErr.message); setSaving(false); return; }
    const userId = signData?.user?.id;
    if (!userId) { setError("No se pudo obtener el ID del usuario"); setSaving(false); return; }
    const { error: linkErr } = await supabase.from("pas_portal_users").insert({ user_id: userId, pas_id: modal.pas_id });
    if (linkErr) { setError("Error al vincular: " + linkErr.message); setSaving(false); return; }
